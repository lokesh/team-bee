...
const geniusPoints = computed(() => 
  Math.floor(geniusPercent.value * props.possiblePoints)
)

const isGenius = computed(() => 
  props.points >= geniusPoints.value
)

const flagStyles = computed(() => {
  const halfFlagWidth = '32px'
  const percent = `${geniusPercent.value * 100}%`
  return {
...
const percentComplete = computed(() => 
  (props.points / props.possiblePoints).toFixed(2)
)
</script>
